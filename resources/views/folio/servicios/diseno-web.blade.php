...
render(function (View $view): View {
    $serviceDefinition = collect(config('site_services'))
        ->firstWhere('slug', 'diseno-web');

    abort_unless($serviceDefinition, 404);

    Head::schema(
        Schema::breadcrumbs()->items([
            'Inicio' => \App\Support\LocalizedRoute::route('home'),
            __('services.web_design.badge') => \App\Support\LocalizedRoute::route('service', ['service' => 'diseno-web']),
        ])
    );
...
<x-layouts.landing
    :title="__('services.web_design.meta_title')"
    :metaDescription="__('services.web_design.meta_description')"
>
    ...
    {!! json_encode([
        ...
        '@type' => 'Service',
        'name' => __('services.web_design.badge'),
        'description' => __('services.web_design.meta_description'),
        'provider' => [
            '@type' => 'Organization',
            'name' => 'AcuarelaSoft',
        ...
        'url' => \App\Support\LocalizedRoute::route('service', ['service' => 'diseno-web']),
    ...
    <section class="relative min-h-[52vh] flex items-center overflow-hidden" aria-labelledby="service-hero-heading">
        ...
        <div class="max-w-6xl mx-auto px-6 pt-28 pb-20 md:pt-36 md:pb-28 w-full">
            ...
            <nav class="mb-8 flex items-center gap-2 font-sans text-sm text-ink/50" aria-label="Ruta de navegación">
                ...
                <span class="text-ink/70">{{ __('services.web_design.badge') }}</span>
            </nav>
            ...
            <div class="grid lg:grid-cols-2 gap-12 items-center">
                <div class="max-w-xl">
                    ...
                    <p class="font-sans text-ink/70 text-lg md:text-xl leading-relaxed mb-5 italic">
                        {{ __('services.web_design.tagline') }}
                    </p>

                    <p class="font-sans text-ink/65 text-base leading-relaxed mb-10">
                        {{ __('services.web_design.description') }}
                    </p>
                ...
                <div class="flex justify-center lg:justify-end">
                    <div class="w-full max-w-sm rounded-[18px] border border-acuarela-300/35 bg-paper/90 backdrop-blur-[3px] p-8 shadow-[0_4px_32px_rgba(46,107,120,0.07)]">
                        <div class="flex flex-col items-center text-center gap-6">
                            ...
                            <div>
                                <p class="font-heading text-ink text-xl font-semibold mb-2">{{ __('services.web_design.badge') }}</p>
                                <p class="font-sans text-ink/60 text-sm leading-relaxed">{{ __('services.web_design.tagline') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    {{-- ============================================================
         WHAT'S INCLUDED (design / optional photo session / hosting)
         ============================================================ --}}
    <section class="relative py-20 px-6 md:py-28 overflow-hidden reveal" aria-labelledby="offering-heading">
        <div class="absolute inset-0 -z-10" aria-hidden="true"
             style="background: radial-gradient(ellipse 70% 55% at 80% 20%, rgba(111,168,216,0.08) 0%, transparent 65%);"></div>

        <div class="max-w-6xl mx-auto">
            <div class="text-center mb-14">
                <h2 id="offering-heading" class="font-heading text-ink text-3xl md:text-4xl font-bold mb-3">{{ __('services.web_design.offering_title') }}</h2>
                <p class="font-sans text-ink/60 text-lg max-w-2xl mx-auto">{{ __('services.web_design.offering_subtitle') }}</p>
            </div>

            <ul class="grid lg:grid-cols-3 gap-8 list-none">
                {{-- Step 1: website design (included) --}}
                <li>
                    <article class="flex flex-col h-full bg-paper rounded-soft p-8 border border-acuarela-400/15 transition-all duration-250 hover:border-acuarela-400/30 hover:-translate-y-0.5">
                        <span class="inline-flex self-start items-center gap-2 px-3 py-1 rounded-pill bg-mint/20 text-petroleo font-sans text-xs font-semibold mb-5">
                            Incluido
                        </span>
                        <h3 class="font-heading text-ink text-xl font-semibold mb-3">{{ __('services.web_design.step_design_title') }}</h3>
                        <p class="font-sans text-ink/70 text-base leading-relaxed grow">{{ __('services.web_design.step_design_desc') }}</p>
                        <p class="mt-6 font-heading text-petroleo text-lg font-semibold">{{ __('services.web_design.step_design_price') }}</p>
                    </article>
                </li>

                {{-- Step 2: optional photography session, CDMX only --}}
                <li>
                    <article class="flex flex-col h-full bg-paper rounded-soft p-8 border border-acuarela-400/15 transition-all duration-250 hover:border-acuarela-400/30 hover:-translate-y-0.5">
                        <span class="inline-flex self-start items-center gap-2 px-3 py-1 rounded-pill bg-salmon/15 text-petroleo font-sans text-xs font-semibold mb-5">
                            {{ __('services.web_design.step_photo_badge') }}
                        </span>
                        <h3 class="font-heading text-ink text-xl font-semibold mb-3">{{ __('services.web_design.step_photo_title') }}</h3>
                        <p class="font-sans text-ink/70 text-base leading-relaxed grow">{{ __('services.web_design.step_photo_desc') }}</p>
                        <p class="mt-6 font-heading text-petroleo text-lg font-semibold">{{ __('services.web_design.step_photo_price') }}</p>
                    </article>
                </li>

                {{-- Step 3: optional managed hosting, monthly or annual --}}
                <li>
                    <article class="flex flex-col h-full bg-paper rounded-soft p-8 border border-acuarela-400/15 transition-all duration-250 hover:border-acuarela-400/30 hover:-translate-y-0.5">
                        <span class="inline-flex self-start items-center gap-2 px-3 py-1 rounded-pill bg-acuarela-400/10 text-petroleo font-sans text-xs font-semibold mb-5">
                            Opcional
                        </span>
                        <h3 class="font-heading text-ink text-xl font-semibold mb-3">{{ __('services.web_design.step_hosting_title') }}</h3>
                        <p class="font-sans text-ink/70 text-base leading-relaxed">{{ __('services.web_design.step_hosting_desc') }}</p>
                        <ul class="mt-4 space-y-1.5 list-none">
                            @foreach (__('services.web_design.step_hosting_features') as $hostingFeature)
                                <li class="flex items-center gap-2 font-sans text-ink/65 text-sm">
                                    <svg class="w-4 h-4 text-petroleo shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/></svg>
                                    {{ $hostingFeature }}
                                </li>
                            @endforeach
                        </ul>
                        <p class="mt-6 font-heading text-petroleo text-lg font-semibold">
                            {{ __('services.web_design.step_hosting_price_monthly') }}
                            <span class="text-ink/50 text-sm font-sans font-normal"> · {{ __('services.web_design.step_hosting_price_annual') }}</span>
                        </p>
                    </article>
                </li>
            </ul>

            <p class="mt-8 text-center font-sans text-ink/50 text-sm">{{ __('services.web_design.pricing_disclaimer') }}</p>
        </div>
    </section>
            ...
            <ul class="grid sm:grid-cols-2 gap-8 list-none">
                <li>
                    <article class="bg-paper rounded-soft p-8 border border-acuarela-400/15 transition-all duration-250 hover:border-acuarela-400/30 hover:-translate-y-0.5 h-full">
                        ...
                        <h3 class="font-heading text-ink text-lg font-semibold mb-2">{{ __('services.web_design.feature_1_title') }}</h3>
                        <p class="font-sans text-ink/70 text-base leading-relaxed">{{ __('services.web_design.feature_1_desc') }}</p>
                    </article>
                </li>
                <li>
                    <article class="bg-paper rounded-soft p-8 border border-acuarela-400/15 transition-all duration-250 hover:border-acuarela-400/30 hover:-translate-y-0.5 h-full">
                        ...
                        <h3 class="font-heading text-ink text-lg font-semibold mb-2">{{ __('services.web_design.feature_2_title') }}</h3>
                        <p class="font-sans text-ink/70 text-base leading-relaxed">{{ __('services.web_design.feature_2_desc') }}</p>
                    </article>
                </li>
                <li>
                    <article class="bg-paper rounded-soft p-8 border border-acuarela-400/15 transition-all duration-250 hover:border-acuarela-400/30 hover:-translate-y-0.5 h-full">
                        ...
                        <h3 class="font-heading text-ink text-lg font-semibold mb-2">{{ __('services.web_design.feature_3_title') }}</h3>
                        <p class="font-sans text-ink/70 text-base leading-relaxed">{{ __('services.web_design.feature_3_desc') }}</p>
                    </article>
                </li>
                <li>
                    <article class="bg-paper rounded-soft p-8 border border-acuarela-400/15 transition-all duration-250 hover:border-acuarela-400/30 hover:-translate-y-0.5 h-full">
                        ...
                        <h3 class="font-heading text-ink text-lg font-semibold mb-2.5">{{ __('services.web_design.feature_4_title') }}</h3>
                        <p class="font-sans text-ink/70 text-base leading-relaxed">{{ __('services.web_design.feature_4_desc') }}</p>
                    </article>
                </li>
            </ul>
        </div>
        ...
        <div class="max-w-4xl mx-auto">
            ...
            <ol class="relative ml-4 space-y-10 list-none border-l-2 border-acuarela-400/20">
                <li class="pl-10 relative">
                    ...
                    <p class="font-sans text-ink/75 text-base leading-relaxed pt-1">{{ __('services.web_design.process_1') }}</p>
                </li>
                <li class="pl-10 relative">
                    ...
                    <p class="font-sans text-ink/75 text-base leading-relaxed pt-1">{{ __('services.web_design.process_2') }}</p>
                </li>
                <li class="pl-10 relative">
                    ...
                    <p class="font-sans text-ink/75 text-base leading-relaxed pt-1">{{ __('services.web_design.process_3') }}</p>
                </li>
            </ol>
        </div>
    </section>
    ...
    <section class="relative py-24 px-6 md:py-32 overflow-hidden reveal" aria-labelledby="cta-heading">
        ...
        <div class="max-w-2xl mx-auto text-center">
            ...
            <h2 id="cta-heading" class="font-heading text-ink text-3xl md:text-4xl font-bold mb-4 leading-tight">
                {{ __('services.web_design.cta_title') }}
            </h2>
